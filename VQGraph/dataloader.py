...
def load_out_t(out_t_dir, name):
    path = out_t_dir.joinpath(name)
    data = np.load(path)
    if "arr_0" in data:
        return torch.from_numpy(data["arr_0"])
    elif name.endswith(".npz"):
        # Support named keys like soft_token_assignments
        return torch.from_numpy(next(iter(data.values())))
    else:
        raise KeyError(f"{name} has no usable key in .npz file")
